...
TEXT = environ.get(" {mention} /nJoined {title} \n 👤 User\n├ id: {joinid}\n ├ first_name : {firstname}\n├ last_name: {lastname}\n├ username: {username}\n├ Premium: {premium} ", " {mention}-{joinid} Joined {title}")
...
async def autoapprove(client: pr0fess0r_99, message: ChatJoinRequest):
    ...
    if APPROVED == "on":
        await client.send_message(chat_id=-1001795651814, text=TEXT.format(mention=user.mention, title=chat.title, joinid=user.id, firstname=user.first_name, lastname=user.last_name, username=user.username, premium=user.is_premium ))
    #   print("Welcome....")
